...
No link extraction, automatic enqueue, Sitemap, History API discovery, button discovery, pagination, infinite scroll, asset download, HTML rewrite, API capture, authentication, or human-paced interaction is present inside the Rendering Engine. Phase 18 consumes the original rendered.html as a separate deterministic transformation; it does not alter the rendering stages or execute rewritten content. A bounded scroll hook exists only in deterministic fixture mode and requires an explicit completion selector.
