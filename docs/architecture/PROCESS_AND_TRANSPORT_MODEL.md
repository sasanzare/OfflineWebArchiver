...
CLI parses bounded commands, constructs contract `1.9.0`, invokes Application Service in process, writes redacted logs to stderr, and returns human or JSON data on stdout. Browser Process, Interaction, Session, and Secret Store ownership remain inside one Application Service/privileged main boundary; `service.close()` locks/disposes Secret Stores and closes other resources. Desktop inspection/mutations use the same two-method bridge, exact path grant, and approved command-type allowlist as Project operations. Raw typed text, secret values, passphrases, and Playwright handles never cross the general transport boundary.
