...
CLI parses bounded commands, constructs contract `1.6.0`, invokes Application Service in process, writes redacted logs to stderr, and returns human or JSON data on stdout. Browser Process and Interaction ownership remain inside one Application Service instance; `service.close()` closes them. Desktop inspection/mutations use the same two-method bridge and exact path grant as Project operations. Raw typed text and Playwright handles never cross the transport boundary.
