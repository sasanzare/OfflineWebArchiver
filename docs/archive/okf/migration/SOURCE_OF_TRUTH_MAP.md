...
| Extension contract and interpretation | `okf-extension/README.md` | Migration design documents | OKF maintainers | Link and canonical-path checks |
| Extension version and registry locations | `okf-extension/manifest.json` | Documentation summaries | Validator maintainers | Manifest schema and procedural checks |
| Stable extension identifiers | `okf-extension/registry/nodes.json`, `domains.json`, `phases.json` | Concept `owa` references | Knowledge maintainers | Unique-ID and canonical-path checks |
| Machine-readable evidence | `okf-extension/registry/evidence.json` | Official `sources`; extension evidence reports | Evidence owners | Evidence target, path, status, and orphan checks |
| Typed relationships | `okf-extension/registry/relationships.json` | Human-readable Markdown links | Knowledge maintainers | Unique-edge and endpoint checks |
...
| Extension change history | `okf-extension/registry/changes.json` | Phase implementation reports | OKF maintainers | Schema, ID, and path checks |
...
| Extension data shapes | Two production schemas under `okf-extension/validation/schemas/` | Eight design schemas under `docs/okf-migration/schema/` | Validator maintainers | Parse, unique `$id`, and local `$ref` checks |
