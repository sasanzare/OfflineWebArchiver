...
| Extension JSON compatibility paths | Eleven JSON files remain at the established manifest, registry, and schema paths | One manifest, eight registries, and two schemas exist and are consumed by the extension validator | `okf/manifest.json`; `okf/registry/*.json`; `okf/validation/schemas/*.json`; `tools/okf/validate.mjs` | CONFIRMED | Retain the established JSON paths as final project extensions unless a consumer-safe relocation is proven necessary | Medium |
