...
Extension Markdown is realized in `okf-extension/`; legacy paths remain notices. A physical registry move is deferred to Phase 6 because the current validator requires `okf-extension/manifest.json`, `okf-extension/registry/*.json`, and two legacy validation Markdown paths.
