...
| AC-P13-019 | NFR-REL-001, NFR-REL-002 | SQLite concurrency stress plan | Run current SQLite races and review crash/commit/fencing matrix | Current ledgers remain consistent; Phase 16 scheduler state is covered by focused persistence tests, while long-running target soak remains later work | persistence/concurrency suite; scheduler lifecycle test | Critical | PASS |
...
### Product Phase 16 execution acceptance

These rows reconcile the current user-directed Phase 16 boundary. `PASS` means
the local implementation and focused evidence for the stated boundary pass; it
does not promote the later authorized target-site or clean-HEAD release gates.

| ID | Requirement IDs | Area | Verification | Evidence | Criticality | Status |
|---|---|---|---|---|---|---|
| AC-P16-001 | FR-RATE-001, NFR-PERF-001 | Versioned scheduler policy | Validate explicit concurrency, rate, burst, cooldown, lease, and heartbeat bounds | `tests/unit/scheduler.test.ts` | Critical | PASS |
| AC-P16-002 | FR-RATE-001 | Global Worker limit | Reserve more Jobs than the configured global cap and run bounded executors | `tests/unit/scheduler.test.ts` | Critical | PASS |
| AC-P16-003 | FR-RATE-001 | Origin and proxy limits | Enforce per-origin Page capacity, per-proxy capacity, and proxy metadata capacity | `tests/unit/scheduler.test.ts` | Critical | PASS |
| AC-P16-004 | FR-RATE-001, NFR-PERF-001 | Origin network budget | Enforce in-flight permits, optional token bucket, burst, and observable wait/backpressure | `packages/archive-core/src/scheduler.ts`; scheduler tests | Critical | PASS |
| AC-P16-005 | FR-PROXY-003 | Sticky/fail-closed affinity | Reject proxy conflicts, unavailable bound proxies, and direct-mode proxy-bound Jobs | `tests/unit/scheduler.test.ts`; existing Session tests | Critical | PASS |
| AC-P16-006 | FR-RATE-002 | Shared `429` cooldown | Record a headerless `429` and block the canonical Origin before any proxy selection | scheduler anti-evasion test | Critical | PASS |
| AC-P16-007 | FR-RATE-002 | Bounded `Retry-After` | Parse delta-seconds, HTTP-date, invalid, past, and oversized values without early dispatch | `tests/unit/scheduler.test.ts` | Critical | PASS |
| AC-P16-008 | NFR-SEC-001 | No alternate-proxy bypass | Keep alternate proxies blocked during the shared Origin cooldown | `tests/unit/scheduler.test.ts` | Critical | PASS |
| AC-P16-009 | NFR-REL-002 | Durable cooldown state | Persist one canonical Project/Run/Origin row and restore it before dispatch | `tests/integration/scheduler-lifecycle.test.ts`; migration 011 | Critical | PASS |
| AC-P16-010 | FR-RENDER-001, NFR-PERF-001 | Browser Context ownership | Allow multiple isolated Worker Contexts and reject restart while any is active | `tests/browser/browser-runtime.test.ts` | Critical | PASS |
| AC-P16-011 | NFR-SEC-002, NFR-SEC-003 | Network permit lifecycle | Keep Browser Runtime as the Playwright owner and release GET/HEAD permits on terminal paths | `packages/browser-runtime/src/index.ts`; browser suite | Critical | PASS |
| AC-P16-012 | NFR-KNOW-001, NFR-KNOW-002, NFR-KNOW-003, NFR-KNOW-004 | Knowledge reconciliation | Synchronize report, ADR, architecture, risks, traceability, OKF, and HANDOFF | docs/OKF validators and HANDOFF | High | PASS |

Phase 16 is implemented and locally validated within these rows. Exact clean
