...
test("opening schema version 1 creates a verified backup and migrates forward", async () => {
  ...
  try {
    ...
    const database = new DatabaseSync(databasePath);
    database.exec("DROP INDEX replay_snapshots_lookup; DROP TABLE replay_runtime_events; DROP TABLE replay_snapshots; DROP INDEX page_asset_relations_source; DROP INDEX page_asset_relations_page; DROP INDEX asset_sources_claim; DROP INDEX asset_sources_run_state; DROP TABLE page_asset_relations; DROP TABLE asset_sources; DROP TABLE asset_contents; DROP INDEX origin_rate_limits_due; DROP TABLE origin_rate_limits; DROP INDEX proxies_project_updated; DROP INDEX proxies_project_health; DROP TABLE proxies; DROP INDEX browser_sessions_project_validation; DROP INDEX browser_sessions_project_state; DROP TABLE browser_sessions; DROP INDEX interaction_traces_fencing; DROP INDEX interaction_traces_job_time; DROP INDEX interaction_profiles_project_revision; DROP TABLE interaction_traces; DROP TABLE interaction_profiles; DROP TABLE render_failures; DROP TABLE render_events; DROP TABLE render_results; DROP TABLE execution_sessions; DROP TABLE recovery_events; DROP TABLE recovery_operations; DROP TABLE completed_outputs; DROP TABLE artifact_checkpoints; DROP TABLE run_checkpoints; DROP TABLE job_checkpoints; DROP TABLE job_leases; DROP TABLE run_control; DROP TABLE queue_operations; DROP TABLE job_discoveries; DROP TABLE job_transitions; DROP TABLE job_attempts; DROP TABLE page_jobs; DROP TABLE scope_decisions; DROP INDEX site_profile_revisions_profile_sequence; DROP TABLE scope_rules; DROP TABLE site_profile_revisions; DROP TABLE site_profiles; DROP INDEX project_events_project_time; DROP TABLE project_events; DELETE FROM schema_migrations WHERE sequence > 1; PRAGMA user_version = 1; UPDATE project_metadata SET schema_version = 1");
    database.close();
    ...
    assert.equal(opened.schemaVersion, 13);
    await storage.close();
