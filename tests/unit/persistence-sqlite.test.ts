...
test("schema 10 installs Lease, Checkpoint, Recovery, Render, Interaction, Session, Run State, and Proxy ledgers", () => {
  ...
  for (const name of ["page_jobs_claim_order", "page_jobs_retry_due", "page_jobs_state", "job_attempts_job_number", "job_transitions_job_time", "job_discoveries_child_time", "queue_operations_project_time", "render_results_job_created", "render_events_job_sequence", "render_failures_job_time", "interaction_profiles_project_revision", "interaction_traces_job_time", "interaction_traces_fencing", "browser_sessions_project_state", "browser_sessions_project_validation", "proxies_project_health", "proxies_project_updated"]) assert.equal(indexes.has(name), true, name);
