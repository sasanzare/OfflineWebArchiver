...
test("built CLI executes Project, Profile, Scope, Queue, and Recovery operations", () => {
  ...
  try {
    ...
    assert.equal(JSON.parse(describe.stdout).result.coreStatus, "rendering-engine-ready");
    const browserInfo = run(["browser", "info", "--json"]);
    assert.equal(browserInfo.status, 0, browserInfo.stderr);
    assert.equal(JSON.parse(browserInfo.stdout).result.info.systemBrowserFallback, false);
    const browserValidate = run(["browser", "validate", "--json"]);
    assert.equal(browserValidate.status, 0, browserValidate.stderr);
    assert.equal(JSON.parse(browserValidate.stdout).result.info.valid, true);
    assert.equal(run(["browser", "health", "--json"]).status, 0);
