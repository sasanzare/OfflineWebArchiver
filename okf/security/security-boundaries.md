...
Project and ZIP paths, Scope and private-network controls, Queue ownership, and Recovery controls remain active. Interaction targets, traces, Cookie Banner rules, Dialogs, Popups, Session validation, and proxy-required work fail closed or redact by default. Phase 19 selective Network Replay capture, exact mapped loopback Local Runtime serving, Strict Offline enforcement, and explicit Service Worker policy resolution are implemented within their declared local boundary. Production Application Service preview orchestration, target-site evidence, DNS connection pinning, screenshot/trace retention, hostile final-archive execution, OTP automation, Worker Pool rate compliance, automatic proxy rotation, release signing, Phase 9 discovery, Phase 20 behavior, and native Linux/macOS evidence remain incomplete or planned. Phase 15's generated local HTTPS certificate exception is test-only and gated by `OWAB_TEST_MODE=1`; production TLS validation remains strict.
