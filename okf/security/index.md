# Security

<!-- MAINTAINED NAVIGATION. Update when direct Concept children change. -->

This directory contains the security controls that constrain Scope, Browser runtime, and protected authentication behavior.

- [Runtime Network Policy](runtime-network.md) - Request authorization and redirect revalidation.
- [Security Boundaries](security-boundaries.md) - Active controls and incomplete boundaries.
- [Proxy Security](proxy-security.md) - Credential, routing, health, and fail-closed controls for Phase 15.
- [Authentication Session Security](authentication-sessions.md) - No-capture, protected-storage, validation, and isolation controls.
- [Asset Path Safety](asset-path-safety.md) - Canonical Project-root, symlink, lock, and atomic-promotion controls for Phase 17.
- [HTML Rewrite Boundary](html-rewrite-boundary.md) - Untrusted-content, URL-scheme, canonical-path, provenance, and atomic-output controls for Phase 18.
