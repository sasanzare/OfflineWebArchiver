...
Each attempt receives a fresh deterministic Context and Page. Windows 11 x64
is the current native release target; Windows 10 is legacy/compatibility, and
Linux and macOS packaging remain future-version work. Browser lifecycle is
consumed by the [Rendering](../workflow/rendering.md) workflow and is started
by the [Application Service](application-service.md) only for an eligible
queued Job.

The renderer does not receive a raw Browser privilege from the application boundary. The partial [Browser-Native Human-Paced Interaction](browser-interaction.md) component is the only approved-plan input boundary; it reuses the fixed Context and keeps raw handles inside Browser Runtime. Product Phase 12 adds a separate headed manual Authentication Context and a fresh headless restored Context. Both use the approved Profile, sandbox, HTTPS validation, disabled downloads, and explicit document-origin authorization; only the authentication Context may remain visible for user-driven provider redirects. Storage State capture/restoration stays behind the Browser Runtime port and Phase 11 Secret Store boundary. Phase 15 maps validated HTTP, HTTPS, and SOCKS5 proxy configurations to Playwright and runs isolated connectivity checks with no direct fallback. The generated local HTTPS fixture may use a certificate exception only under the test-only `OWAB_TEST_MODE=1` boundary; normal contexts remain strict. Network authorization and result persistence remain governed by the [Contracts](contracts.md), [Queue](../workflow/queue.md), and [Checkpoint Recovery](../recovery/checkpoint-recovery.md) Concepts.
